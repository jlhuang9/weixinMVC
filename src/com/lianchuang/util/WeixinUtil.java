...
//			PropertiesUtil putil=new PropertiesUtil(Constants.CONF_PROPERTIES_WEB_PATH);
//			System.out.println(putil.getProperty("ACCESS_TOKEN"));
...
//				//超时从新获取accessToken
//				token = getAccessToken2(Constants.APPID, Constants.SECRET);
...
public static String gettoken(){
	...
	try {
		//获取accessToken之前判断文件里有没有，没有重新获取，填写过期时间，有判断时间有没有过期
		Long overduetime = Long.parseLong(Constants.OVERDUE_ACCESS_TOKEN_TIME);
		Timestamp s = new Timestamp();
		AccessToken token = new AccessToken() ;
		if(s.getDateTime()>overduetime){
			//超时从新获取accessToken
			token = getAccessToken2(Constants.APPID, Constants.SECRET);
			Constants.OVERDUE_ACCESS_TOKEN_TIME=String.valueOf(s.getDateTime()+Constants.EFFECTIVE_TIME);
			Constants.ACCESS_TOKEN = token.getAccessToken();
			temp = Constants.ACCESS_TOKEN;
		}else{
			temp = Constants.ACCESS_TOKEN;
		}
	} catch (Exception e) {
